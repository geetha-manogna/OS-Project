...
// Checks if block is free, if yes then marks it as used and clear the block data and returns 1
int bcheck_free_alloc(uint dev, uint block)
{
  struct buf *bp;
  int bi = block % BPB;
  int m = 1 << (bi % 8);
...
bmap(struct inode *ip, uint bn)
{
  ...
  if (ip->type == T_EXTENT)
  {
    ...
    for (i = 0; i < extent_addrs_size; i += 2)
    {
      ...
      {
        block_within_extent = bn - acc_blocks;
        extent_start = ip->addrs[i];
        return extent_start + block_within_extent * BSIZE;
      }
      acc_blocks += ip->addrs[i + 1];
    }
    ...
    for (i = 0; i < extent_addrs_size; i += 2)
    {
      ...
      {                         // Find an unused extent entry
        addr = balloc(ip->dev); // Allocate a single block
        if (addr == 0)
        {
          panic("bmap: out of disk space");
        }
        ip->addrs[i] = addr;
        ip->addrs[i + 1] = 1;
        return addr;
    ...
    panic("bmap: no space in inode for new extent");
  }
  ...
  else
  {
    ...
    for (int ind = 0; ind < 2; ind++)
    {
      if (bn < NDINDIRECT)
      {
        if ((addr = ip->addrs[NDIRECT + 1 + ind]) == 0)
          ip->addrs[NDIRECT + 1 + ind] = addr = balloc(ip->dev);
        bp = bread(ip->dev, addr);
        a = (uint *)bp->data;
        uint indirect_index = bn / NINDIRECT;
        uint indirect_offset = bn % NINDIRECT;
...
void stati(struct inode *ip, struct stat *st)
{
  ...
  st->nlink = ip->nlink;
  st->size = ip->size;
  // cprintf("geetha came here\n");
  memmove(st->addrs, ip->addrs, sizeof(st->addrs));
  // cprintf("geetha came here1\n");
}
