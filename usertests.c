...
void symlinktest(void)
{
  int fd;
  printf(1, "symlinktest\n\n");

  printf(1, "Created a file sl1 with text=hello.\n\n");
  fd = open("sl1", O_CREATE | O_RDWR);
  if (fd < 0)
  {
    printf(1, "create sl1 failed\n");
    exit();
  }
  if (write(fd, "hello", 5) != 5)
  {
    printf(1, "write sl1 failed\n");
    exit();
  }
  close(fd);
  ...
  {
    printf(1, "symlink sl1 sl2 failed\n");
    exit();
  }
  ...
  printf(1, "Creating symlink with depth=2.\n");
  if (symlink("sl2", "sl3") < 0)
  {
    printf(1, "symlink sl2 sl3 failed\n");
    exit();
  }
  ...
  printf(1, "Pointing symlink to a file that does not exist\n");
  if (symlink("random_not_exists", "sl4") < 0)
  {
    printf(1, "symlink random_not_exists sl4 failed\n");
    exit();
  }
  // This should not open since the file sl4 is referring to is not present
  fd = open("sl4", 0);
  if (fd >= 0)
  {
    printf(1, "sl4 referred to a non-existent file, still it opened.\n");
    exit();
  }
  else
  {
    printf(1, "open() call failed(as expected) when symlink points to non-existent file\n\n");
  }

  printf(1, "Linking a file using link system call\n");
  fd = open("sl5", O_CREATE | O_RDWR);
  if (fd < 0)
  {
    printf(1, "create sl5 failed\n");
    exit();
  }
  if (write(fd, "test", 4) != 4)
  {
    printf(1, "write sl5 failed\n");
    exit();
  }
  close(fd);
  ...
  if (link("sl5", "sl6") < 0)
  {
    printf(1, "link sl5 sl6 failed\n");
    exit();
  }

  printf(1, "Opening the file using O_NOFOLLOW\n");
  fd = open("sl6", O_NOFOLLOW);
  if (fd < 0)
  {
